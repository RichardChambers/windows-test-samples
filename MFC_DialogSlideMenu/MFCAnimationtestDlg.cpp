...
//		fadeInThread = std::thread(&CMFCAnimationtestDlg::FadeIn, this, GetDlgItem(IDD_MFCANIMATIONTEST_NOTES));
	}
}
...
BOOL CMFCAnimationtestDlg::OnInitDialog()
{
	...
	if (pSlideHolder) {
		pSlidePane = new CSlidePane(this);
		pSlidePane->CreateSlideBar(pSlideHolder, this);
	}

#if 1
	m_Notes.AttachClientArea(this, IDD_MFCANIMATIONTEST_NOTES);

	m_Notes.SetWindowText(L"The attache worked.");

	m_Notes.PrintClientArea(CPoint(10, 20), L"Line 1");
	m_Notes.PrintClientArea(CPoint(10, 40), L"Line 2");
	m_Notes.PrintClientArea(CPoint(10, 60), L"Line 3");

	m_Notes.UpdateClientArea();
